...
export async function findTicketById(id: string): Promise<DbTicket | null> {
  const rows = await rest<DbTicket[]>(`/rest/v1/Ticket?select=id,ticketNumber,title,description,status,priority,phone,company,clientName,reportTo,solution,attachmentUrl,creatorId,assigneeId,createdAt,updatedAt,creator:User!Ticket_creatorId_fkey(id,name,email),assignee:User!Ticket_assigneeId_fkey(id,name,email)&id=eq.${encodeURIComponent(id)}&limit=1`)
  return rows[0] || null
}
